...
jQuery.fn.validationEngine = function (settings) {
    if ($.validationEngineLanguage) {					// IS THERE A LANGUAGE LOCALISATION ?
        ...
        allRules = {
            ...
            "telephone": {
                "regex": "/^[0-9\-\(\)]{12}$/",
                "alertText": "Incorrect phone number! Example380971138382"
            },
            "email": {
            ...
            "onlyLetter": {
                "regex": "/^[а-яА-Я]+|[a-zA-Z]+$/",
                "alertText": "Только  буквы и только одного языка!"
            },
            "password": {
                "regex": "/(?=^.{6,255}$)((?=.*\d)(?=.*[A-Z])(?=.*[a-z])|(?=.*\d)(?=.*[^A-Za-z0-9])(?=.*[a-z])|(?=.*" +
                "[^A-Za-z0-9])(?=.*[A-Z])(?=.*[a-z])|(?=.*\d)(?=.*[A-Z])(?=.*[^A-Za-z0-9]))^.*/",
                "alertText": "Password must be at least include at least 1 upper case letter," +
                " 1 lower case letter,1 numeric digit and 1 special characte!"
            }
        }
    }
    ...
    $("#formID").bind("submit", function (caller) {   // ON FORM SUBMIT, CONTROL AJAX FUNCTION IF SPECIFIED ON DOCUMENT READY
        if (submitValidation(this) == false) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#popup').html('Registratin succsed!').fadeOut(1500, function () {
                        $('#popup').html(form);
                    });
                    $('#layer').fadeOut(1000);
                },
                error: function(xhr, status, error) {
                    ...
                    alert(d.substring(d.indexOf("<h1>")+22,d.indexOf("</h1>")));
                }
            });
            if (settings.success) {
